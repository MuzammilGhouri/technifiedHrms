<?php
  ...
  { 
    ...
    public function processApply(Request $request)
    {
      ...
      $remaining_leave = EmployeeLeaves::where('user_id',\Auth::user()->id)->where('leave_type_id',$request->leave_type)->select('days')->get();

      $total = 0;
      
      foreach($remaining_leave as $leave){
          
         $total += (int)$leave->days ;  
            ...
            if($total + $request->number_of_days > 10 && $request->leave_type == 1){
                \Session::flash('flash_message', 'Your Leaves Are Exceed');
                return redirect()->back();
            }
            if($total + $request->number_of_days > 15 && $request->leave_type == 2){
                \Session::flash('flash_message', 'Your Leaves Are Exceed');
                return redirect()->back();
            }
            if($total + $request->number_of_days > 20 && $request->leave_type == 3){
                \Session::flash('flash_message', 'Your Leaves Are Exceed');
                return redirect()->back();
            } 
            ...
            $empLeave = Employee::where('user_id', \Auth::user()->id)->with('user')->first();
            
            
            $teams = Team::where('id',$empLeave->team_id)->first();
          
          
            if($teams) {
                $tl_id = $teams->leader_id;
            ...
            $leave->save();
            $leave_form_id = $leave->id;
            // dd($leave->id);
            $sendTo = [$leave->tl_id,$leave->manager_id,'1'];    
            $Users = User::whereIn('id', $sendTo)->get();
            $leaveDetail  = DB::table('employee_leaves')->where('user_id',auth()->user()->id)->orwhere('id',$leave_form_id)->orderBy('id', 'desc')->first();
                            
            
            foreach($Users as $user){ 
                
                $user->notify(new UserNotification($leaveDetail));
    ...
    public function showMyLeave()
    {
      ...
      $leaves = EmployeeLeaves::where('user_id', \Auth::user()->id)->get();
        $remaining_casual_leave = EmployeeLeaves::where('user_id',\Auth::user()->id)->where('leave_type_id',1)->where('hr_approval',1)->where('manager_approval',1)->select('days')->get();
        $remaining_sick_leave = EmployeeLeaves::where('user_id',\Auth::user()->id)->where('leave_type_id',2)->where('hr_approval',1)->where('manager_approval',1)->select('days')->get();
        $remaining_annual_leave = EmployeeLeaves::where('user_id',\Auth::user()->id)->where('leave_type_id',3)->where('hr_approval',1)->where('manager_approval',1)->select('days')->get();
        ...
        foreach($remaining_sick_leave as $leave){  
         $total_sick_leave += (int)$leave->days ;  
        }
        foreach($remaining_annual_leave as $leave){  
         $total_annual_leave += (int)$leave->days ;  
        }
       
      return view('hrms.leave.show_my_leaves', compact('leaves','total_casual_leave','total_annual_leave','total_sick_leave'));
    }
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function showAllLeave(Request $request,$id)
    {   
      ...
      return view('hrms.leave.total_leave_request', compact('leaves', 'column','employees', 'string','dateFrom','dateTo'));
    }
    ...
    public function changeStatus($id){
        ...
        $employeeLeave = EmployeeLeaves::where('id', $id)->first();
        
        return view('hrms.leave.change_status',compact('employeeLeave'));
    }
    ...
    public function approveLeave(Request $request,$id)
    {
        ...
        if(\Auth::user()->isHR()){
            \DB::table('employee_leaves')->where('id', $id)->update(['hr_approval' => $request->status, 'remarks' => $remarks ,'status'=>$request->status]);
        }elseif(\Auth::user()->isManager()){
            \DB::table('employee_leaves')->where('id', $id)->update(['manager_approval' => $request->status, 'remarks' => $remarks ,'status'=>$request->status]);
        }elseif(\Auth::user()->isteamLaad()){
             \DB::table('employee_leaves')->where('id', $id)->update(['team_lead_approval' => $request->status, 'remarks' => $remarks ,'status'=>$request->status]);
        }
        ...
        $leaveDetail  = DB::table('employee_leaves')->where('user_id',$user->id)->orwhere('id',$id)->first();
        
        $user->notify(new notificationStatus($leaveDetail));
        \Session::flash('flash_message', 'Leave Status Updated Successfully!');
        return redirect()->back();
    }
    ...
    public function processHolidays(Request $request)
    {
      ...
      public function leaveDistinct(Request $request){
        ...
        { 
            ...
            $leaves = EmployeeLeaves::with('user.employee')->distinct()->get(['user_id']);
